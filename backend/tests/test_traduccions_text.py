...
Les versions en anglès no les escriu ningú d'aquí: són les que el web del
projecte ja publica, i hi arriben amb `scripts/scraper_traduccions_textos.py`.
Per això no hi ha endpoint per crear-les -no hi ha panell d'edició-, i aquí es
...
def test_cau_al_catala_quan_lobra_no_esta_traduida(client, db_session, text):
    """Dues de les disset obres no estan traduïdes al web. S'ha de veure
    l'original i s'ha de saber que ho és."""
    _tradueix(db_session, text)
...
def test_lobra_origen_no_es_tradueix(client, db_session, text):
    """Anomena el llibre publicat, que va sortir en català."""
...
def test_el_panell_dedicio_segueix_veient_loriginal(client, db_session, text):
    """El panell demana el text sense idioma. Si li arribés la traducció, el
...
def test_la_traduccio_no_es_desa_a_la_taula_del_text(client, db_session, text):
    """`aplica_idioma` toca l'objecte just abans de serialitzar-lo: si això
    arribés a la base de dades, l'original desapareixeria."""
